...
static bool transferFile(const char* filepath, uint32_t offset, uint32_t length)
{
  ...
  printPercentage(NULL,           // prefix string (ignored)
                  ...
                  endOffset);     
  bootloadPrintf("EBL data start: 0x%08X, end: 0x%08X, size: %d bytes\n",
                  offset,
                  endOffset,
                  endOffset - offset);
  ...
  while (offset < endOffset) {
    ...
    if (returnedLength != readSize) {
      bootloadPrintf("Failed to read image data at offset 0x%08X\n", offset);
      fclose(fileHandle);
      return false;
    }
    ...
    if (!emSendXmodemData(block, 
      ...
      bootloadPrintf("Failed to send %sdata block to NCP.\n",
