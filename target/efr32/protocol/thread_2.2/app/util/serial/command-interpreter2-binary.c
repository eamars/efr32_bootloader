...
static const EmberCommandEntry *getCommand(uint16_t identifier)
{
  ...
  // TODO: now that name and id are stored as a union, I don't think this second
  // check is needed anymore.
  while (commandFinger != NULL && commandFinger->identifier.name != NULL) {
    if (commandFinger->identifier.id == identifier) {
      break;
    }
  ...
  return (commandFinger->identifier.name == NULL
          ? NULL
          : commandFinger);
}
