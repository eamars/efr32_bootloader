...
static void coapListen(const EmberIpv6Address *address);
                                            ...
                                            uint8_t addressFlags)
{
  ...
  if (validLifetime != 0) {
    ...
    if (status != EMBER_SUCCESS) {
      emberAfCorePrint("ERR: Listening for ICMP on ");
      emberAfCoreDebugExec(emberAfPrintIpv6Address(address));
      emberAfCorePrintln(" failed: 0x%x", status);
    }
    coapListen(address);
                         ...
                         const EmberCoapRequestInfo *info)
{
  ...
  if (state != ADVERTISE) {
    ...
    int32_t data = (int32_t)emberFetchHighLowInt32u(payload);
    ...
    if (reports != NULL) {
      ...
      if (fprintf(reports,
                  ...
                  inet_ntop(AF_INET6, &info->remoteAddress, dst, sizeof(dst)),
                  (long int)data) == -1
          || fflush(reports) != 0) {
        perror(NULL);
      }
