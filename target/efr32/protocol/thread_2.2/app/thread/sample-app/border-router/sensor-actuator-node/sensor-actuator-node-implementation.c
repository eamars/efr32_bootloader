...
// Uncomment the ENABLE_JOIN_OUT_OF_BAND to use the official commissioning app
...
// some helpers for parsing JSON payloads from a subscriber
...
// The network name is not communicated to the the end node when in
// pre-comissioned mode (also called join out-of-band mode) and the network name
// is defaulted to "precommissioned". The network name is communicated to the
// end node when in commissioning mode and default name "precommissioned" is
// updated with the network name.
...
static void joinNetwork(void)
{
  ...
  if (joinAttemptsRemaining > 0) {
    ...
    emberAfCorePrintln("Device idle, press button 1 or 'rejoin-network' command to start a join for EUI \"%s\" and join passphrase \"%s\"",
                       euiString,
                       joinKey);
...
static void getJoinKey(uint8_t *joinKey, uint8_t *joinKeyLength)
{
  ...
  char fixedJoinKey[EMBER_JOIN_KEY_MAX_SIZE + 1] = FIXED_JOIN_KEY;
...
void ledHandler(EmberCoapCode code,
                uint8_t *uri,
                EmberCoapReadOptions *options,
                const uint8_t *payload,
                uint16_t payloadLength,
...
void onOffOutHandler(EmberCoapCode code,
                     uint8_t *uri,
                     EmberCoapReadOptions *options,
                     const uint8_t *payload,
                     uint16_t payloadLength,
  ...
  case BUTTON0:
    ...
    case EMBER_SINGLE_PRESS: {
      ...
      if (state == REPORT_TEMP_TO_SUBSCRIBER ||
        ...
        emberAfCorePrint("Reporting button press to: [");
        emberAfCoreDebugExec(emberAfPrintIpv6Address(&subscriberAddress));
        emberAfCorePrintln("](%d)", subscriberPort);
