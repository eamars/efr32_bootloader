...
#include "app/host/host-buffer-management.h"
...
static void setBufferLength(Buffer buffer, uint16_t newLength, bool fromEnd)
{
  assert(buffer - 1 < bufferContentsIndex);
  uint16_t oldLength = bufferContents[buffer - 1].length;
  if (oldLength == newLength) {
    return;
  }
  assert(newLength < oldLength);
  if (fromEnd) {
    // Copy the content that will remain to front of data.
    uint8_t *pd = bufferContents[buffer - 1].data;
    memmove(pd, pd + oldLength - newLength, newLength);
  }
  bufferContents[buffer - 1].length = newLength;
}

void emSetBufferLength(Buffer buffer, uint16_t newLength)
{
  setBufferLength(buffer, newLength, false);
}

void emSetBufferLengthFromEnd(Buffer buffer, uint16_t newLength)
{
  setBufferLength(buffer, newLength, true);
}
