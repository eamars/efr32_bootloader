...
/**
 * \name SECTION: Silicon Labs Additions
 *
 * This section sets Silicon Labs specific settings.
 * \{
 */
...
// gcc.h defines asm(x) to __asm__(x) which does
// work with the mbedtls code so we undefine it
// here.  mbedtls does a good job using the correct
// asm interface depending on the compiler in use.
#undef asm

#undef memset
#define memset(d,v,l)   MEMSET(d,v,l)
#undef memcpy
#define memcpy(d,s,l)   MEMCOPY(d,s,l)
#undef memmove
#define memmove(d,s,l)  MEMMOVE(d,s,l)
#undef memcmp
#define memcmp(s0,s1,l) MEMCOMPARE(s0,s1,l)

void *emBufferCalloc(size_t count, size_t size);
#endif // EMBER_STACK_H

/**
 * When building for the EFM family of processors utilize the hardware
 * crypto acceleration.
 */
...
#define MBEDTLS_PLATFORM_CALLOC_MACRO  emBufferCalloc /**< Default allocator macro to use, can be undefined */
#define MBEDTLS_PLATFORM_FREE_MACRO      emBufferFree /**< Default free macro to use, can be undefined */
