...
#define LED_DATA_RECEIVE 0
#define LED_ACK_RECEIVE  1
...
uint8_t channel = 0;
uint8_t currentConfig = 0; //default is first in list
bool dataTx   = false; //send a data packet
bool ackTx    = false; //send an ack
...
void changeRadioConfig(uint8_t newConfig)
{
...
void generatePayload(RAIL_TxData_t *txInfo)
{
  if (ackTx)
  {
