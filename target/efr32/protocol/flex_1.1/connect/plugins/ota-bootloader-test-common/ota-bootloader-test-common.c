...
{
  uint16_t blVersion, batVersion;

  emberAfPluginBootloaderInterfaceGetVersion(&blVersion, &batVersion);

  emberAfCorePrintln("bootloader version: %d, BAT version: %d", blVersion, batVersion);
}
...
{
  emberAfCorePrintln("bootloader: flash erase started");

  if ( emberAfPluginBootloaderInterfaceChipErase() ) {
    emberAfCorePrintln("bootloader: flash erase succeeded!");
  }
  else {
    emberAfCorePrintln("bootloader: flash erase failed! wrong part?");
  }
}
