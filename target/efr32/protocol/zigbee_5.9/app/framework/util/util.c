                              ...
                              InterPanHeader *interPanHeader)
{
  ...
  // CSTAT complains about setting a static var to point to a local address.
  // This appears to be OK, it seems that any references to the static
  // will be made before returning from this function call. Quiet the CSTAT
  // warning for now, but revisit.
  //
  //cstat !MEM-stack-global !MISRAC2012-Rule-18.6_b !MISRAC2012-Rule-1.3_r
  ...
  if (interPanHeader == NULL) {
    ...
    if (emberAfDetermineIfLinkSecurityIsRequired(cmd.commandId,
      ...
      sendStatus = emberAfSendDefaultResponse(&cmd, EMBER_ZCL_STATUS_FAILURE);
