...
void emberAfPluginCalendarCommonLoadEnhancedCalendarCommand(void)
{
  ...
  if (index < EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS) {
    ...
    // Seasons Table
    //
    // CSTAT complains that the seasons array bounds is exceeded. This is
    // because the plugin's default number of seasons is fewer than 11.
    // Disabling those checks around this code block for now, but this
    // issue would apply to any of the other calendar arrays if they are
    // sized via plugin options to be smaller than the hardcoded indexing
    // used in these CLI test configurations.
    //
    //cstat -ARR-inv-index -MISRAC2012-Rule-18.1_a
    //
