...
bool emAfPluginCommandRelayPreCommandReceivedCallback(EmberAfClusterCommand* cmd)
{
  ...
  for (i = 0; i < EMBER_AF_PLUGIN_COMMAND_RELAY_RELAY_TABLE_SIZE; i++) {
    ...
    // We do not forward any OTA_BOOTLOAD cluster commands at this point.
    // Further improvement should be adding a clusterId entry in the relay table
    // so that the command forwarding is limited to the specified clusterId.
    if ((deviceMatch)
                                 ...
                                 EmberAfPluginCommandRelayDeviceEndpoint* deviceEndpoint2)
{
  ...
  return euisMatch && endpointsMatch;
}
