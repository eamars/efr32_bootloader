...
// Initialize the color control server

// ZHA requirement:  need to set up default reporting entries for all of the 
// reportable attributes.  For color control server, this includes colorX, 
// colorY, colorTemperature, hue, and saturation.  
static void setUpDefaultReportingTableEntries(void)
{
  ...
  // set up common reporting entry parameters here.
    EmberAfPluginReportingEntry defaultConfiguration = {
      ...
      emberAfEndpointFromIndex(0), //endpoint, only support single endpoint
      ZCL_COLOR_CONTROL_CLUSTER_ID, //clusterId
      0, //attributeId. Will be overwritten below
      CLUSTER_MASK_SERVER, //mask
      EMBER_AF_NULL_MANUFACTURER_CODE, //manufacturerCode
      .data.reported = {
        EMBER_AF_PLUGIN_COLOR_CONTROL_SERVER_MIN_REPORT_INTERVAL, //minInterval
        EMBER_AF_PLUGIN_COLOR_CONTROL_SERVER_MAX_REPORT_INTERVAL, //maxInterval
        1 //reportableChange
      }
    };

  // For any reportable attributes we support, add a reporing table entry.
#ifdef EMBER_AF_PLUGIN_COLOR_CONTROL_SERVER_HSV
    defaultConfiguration.data.reported.reportableChange =
      EMBER_AF_PLUGIN_COLOR_CONTROL_SERVER_HUE_CHANGE;
    defaultConfiguration.attributeId = ZCL_COLOR_CONTROL_CURRENT_HUE_ATTRIBUTE_ID;
    reportEntry = 
    ...
    defaultConfiguration.attributeId = 
      ZCL_COLOR_CONTROL_CURRENT_SATURATION_ATTRIBUTE_ID;
    reportEntry = 
      emAfPluginReportingConditionallyAddReportingEntry(&defaultConfiguration);
    assert(reportEntry != REPORT_FAILED);
    ...
    defaultConfiguration.attributeId = ZCL_COLOR_CONTROL_CURRENT_X_ATTRIBUTE_ID;
    reportEntry = 
      emAfPluginReportingConditionallyAddReportingEntry(&defaultConfiguration);
    assert(reportEntry != REPORT_FAILED);

    defaultConfiguration.attributeId = ZCL_COLOR_CONTROL_CURRENT_Y_ATTRIBUTE_ID;
    reportEntry = 
      emAfPluginReportingConditionallyAddReportingEntry(&defaultConfiguration);
    assert(reportEntry != REPORT_FAILED);
#endif  
