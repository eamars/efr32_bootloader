...
bool emberAfPriceClusterPublishBlockThresholdsCallback(uint32_t providerId,
  ...
  if( i < EMBER_AF_PLUGIN_PRICE_CLIENT_BLOCK_THRESHOLD_TABLE_SIZE ){
    ...
    if( subPayloadControl & BLOCK_THRESHOLD_SUB_PAYLOAD_ALL_TOU_TIERS ){
      ...
      //x = 0;
      //
      // Turn off CSTAT checks re tierNumber as possible out-of-bounds index
      // into tierNumberOfBlockThresholds[]. This looks like it could be a
      // legitimate bug in that the kickout range check on tierNumber above
      // (should that be >=, not >, ...MAX_NUMBER_TIERS? but even for that)
      // doesn't restrict it to the bound of the array.
      //
