...
#include EMBER_AF_API_MICROPHONE_CODEC_MSADPCM
#include EMBER_AF_API_ACCELEROMETER_LED
#include EMBER_AF_API_KEY_MATRIX
#include EMBER_AF_API_INFRARED_LED
#include EMBER_AF_API_INFRARED_LED_UIRD

#ifdef EMBER_AF_PLUGIN_BUZZER
    #define MAKE_SOUND(i, b) halPlayTune_P(i, b)
#else
    #define MAKE_SOUND(i, b) emberAfAppPrintln("~SOUND~")
#endif
...
// From buzzer plugin. Indicating if a tune is being played or not
                                                  ...
                                                  uint8_t pairingIndex)
{
  ...
  if (status == EMBER_SUCCESS) {
    ...
    MAKE_SOUND(happyTune, true);

  } else {
    emberAfAppPrintln("%p %p: 0x%x", "Binding", "failed", status);
    MAKE_SOUND(sadTune, true);
...
void keymatrixEventHandler(void)
{
  ...
  // If voice recording is active we only want to scan the voice button
  // to see if it released as some of the other key matrix pins are shared
  // with the audio codec.
  if (voiceActive) {
    halKeyMatrixScanSingle(KEY_MATRIX_VOICE_KEY);
    keyState = halKeyMatrixGetValue(KEY_MATRIX_VOICE_KEY);
    if (keyState) {
      anyKeyDown = true;
    } else {
      keyReleasedHandler(KEY_MATRIX_VOICE_KEY);
      lastKeyStates[KEY_MATRIX_VOICE_KEY] = keyState;
    }
  } else { // Voice key not pressed
    halKeyMatrixScan();
  ...
  if (bindingActive || anyKeyDown) {
    halSetLed(POWER_LED);
  } else {
    halClearLed(POWER_LED);
  }

  // Only schedule another scan if a key is down or if tune is playing
  if (anyKeyDown || !tuneDone) {
...
static void keyPressedHandler(uint8_t key)
{
  ...
  // transition). First the special keys (SET, STB, TV, VOICE) are handled
  ...
  // Both the SET key and the TV or STB key must be down at the same time to
  // initiate pairing to the selected device
  if (key == KEY_MATRIX_SET_KEY) {
    emberAfDebugPrintln("SET pressed");
    pairingKeyPressed = true;
    if ((stbKeyPressed || tvKeyPressed) && !bindingActive) {
      emberEventControlSetActive(bindingEventControl);
    }
  } else if (key == KEY_MATRIX_STB_KEY) {
  ...
  } else if (key == KEY_MATRIX_VOICE_KEY) {
    emberAfAppPrintln("VOICE pressed");
    voiceActive = true;
    halMicrophoneCodecMsadpcmStart();
...
static void keyReleasedHandler(uint8_t key)
{
  ...
  // Check non-mappable special keys
  if (key == KEY_MATRIX_SET_KEY) {
    emberAfDebugPrintln("SET released");
    ...
    voiceActive = false;
    emberAfAppPrintln("VOICE released");
...
{
  // This function is called when new data is ready from the microphone plugin.
  // This will be called at about 12.0 ms intervals when
  // halMicrophoneCodecMsadpcmStart() has been called and is stopped
  // with halMicrophoneCodecMsadpcmStop()
  EmberRf4ceTxOption txOptions;
  uint8_t pairingIndex = pairingDeviceTable[activeDevice];

  // Transmit data as an a vendor specific profile
  if (pairingIndex != NO_PAIRING) {
    ...
    emberAfRf4ceSendExtended(pairingIndex,
                             EMBER_AF_RF4CE_PROFILE_VENDOR_SPECIFIC_VOICE,
                             RF4CE_VENDOR_ID,
                             txOptions,
                             data,
                             length,
